"""
MetaTFT 数据爬虫 (基于真实 API) - 中文版本（保持原设计）
================================
接口来源（通过抓包获得）：
...
保持原设计：数据文件内保留 TFT ID，用 localization.json 做 ID ↔ 中文映射
...
# ── 中文翻译器（仅用于生成 localization.json）────────────────────────────────

class Translator:
    """翻译器：仅用于生成 localization.json"""
...
class OutputBuilder:
    ...
    def save_all(self, comps: list[Comp], tft_set: str, cluster_id: str, translator: Translator):
        ...
        self._save_localization(translator)
    ...
    def _save_localization(self, translator: Translator):
        """
        localization.json
        ID ↔ 中文名映射表
        """
        ...
        log.info(f"✅ localization.json   → {path}  ({len(translator.id_to_cn)} 条翻译)")
    ...
    def _print_summary(self, comps: list[Comp]):
        ...
        log.info(f"\n{'='*60}")
        log.info(f"版本强度速览 (近{DAYS}天 {RANKS}) - 保持原设计")
        log.info(f"{'='*60}")
        for tier in ["S", "A", "B", "C"]:
            cs = tiers[tier]
